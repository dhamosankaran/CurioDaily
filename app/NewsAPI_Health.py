...
class HealthNewsFetcher:
    def __init__(self):
        ...
        self.preferred_entities = [
            'WHO', 'CDC', 'NIH', 'FDA', 'Mayo Clinic', 'Cleveland Clinic',
            'Johns Hopkins', 'Harvard Medical School', 'WebMD', 'Healthline',
            'MedlinePlus', 'JAMA', 'The Lancet', 'New England Journal of Medicine',
        ...
        self.trusted_sources = [
            'who.int', 'cdc.gov', 'nih.gov', 'fda.gov', 'mayoclinic.org',
            'clevelandclinic.org', 'hopkinsmedicine.org', 'health.harvard.edu',
            'webmd.com', 'healthline.com', 'medlineplus.gov', 'jamanetwork.com',
            'thelancet.com', 'nejm.org', 'heart.org', 'cancer.org', 'unicef.org',
            'medicalnewstoday.com', 'sciencedaily.com/health', 'nytimes.com/health',
            'bbc.com/health', 'reuters.com/health', 'theguardian.com/society/health',
            'washingtonpost.com/health', 'health.com', 'everydayhealth.com',
            'verywellhealth.com', 'psychologytoday.com', 'nature.com/subjects/health-sciences'
        ]

        self.focus_areas = [
            # General Health
            'Preventive Care', 'Public Health', 'Global Health', 'Health Policy',
            'Healthcare Technology', 'Telemedicine', 'Personalized Medicine',

            # Medical Specialties
            'Cardiology', 'Oncology', 'Neurology', 'Pediatrics', 'Geriatrics',
            'Immunology', 'Endocrinology', 'Gastroenterology', 'Dermatology',

            # Mental Health
            'Mental Health', 'Psychology', 'Psychiatry', 'Stress Management',
            'Anxiety Disorders', 'Depression', 'PTSD', 'Addiction',

            # Nutrition and Fitness
            'Nutrition', 'Diet', 'Exercise', 'Weight Management', 'Obesity',
            'Sports Medicine', 'Fitness Technology',

            # Diseases and Conditions
            'Infectious Diseases', 'Chronic Diseases', 'Autoimmune Disorders',
            'Rare Diseases', 'Genetic Disorders', 'Cardiovascular Diseases',
            'Cancer Research', 'Diabetes', 'Alzheimer\'s Disease',

            # Women's Health
            'Women\'s Health', 'Reproductive Health', 'Pregnancy', 'Menopause',

            # Men's Health
            'Men\'s Health', 'Prostate Health', 'Testosterone',

            # Children's Health
            'Pediatric Health', 'Childhood Obesity', 'Vaccinations',

            # Alternative Medicine
            'Alternative Medicine', 'Holistic Health', 'Naturopathy', 'Acupuncture',

            # Health Technology
            'Medical Devices', 'Health Apps', 'Wearable Technology', 'AI in Healthcare',

            # Environmental Health
            'Environmental Health', 'Climate Change and Health', 'Air Pollution',

            # Health Education
            'Health Literacy', 'Patient Education', 'Medical Education'
        ]
    ...
    def generate_dynamic_title(self, highlights):
        ...
        be engaging and specific to health trends or breakthroughs mentioned in the highlights.

        Highlights:
        {highlights}

        Generate a title in the format: "Health [Theme]: [Specific Detail]"
        For example: "Health Breakthrough: New Treatment Shows Promise for Alzheimer's"
        """
    ...
    def generate_summary(self, article):
        ...
        prompt = f"""
        Summarize the following health article in this format:

        🩺 **[CATCHY TITLE]: [Subtitle]**

        🔬 **Health Spotlight:**
        [1-2 sentences highlighting the key health trend, discovery, or medical advice]
        ...
        try:
            response = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a health-savvy AI assistant that creates engaging summaries of health news for social media and newsletters."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=300,
                n=1,
        ...
        <body>
            ...
            <main class="container">
                <section class="highlights">
                    <h2>Key Health Highlights <span class="date">{today_date}</span></h2>
    ...
    def generate_article_html(self, article):
        ...
        for section in sections:
            if section.startswith('🩺'):
                ...
                health_spotlight = section.replace('🔬 **Health Spotlight:**', '').strip()
            elif section.startswith('💡'):
    ...
    def save_html_page(self, html_content, filename="health_news_highlights.html"):
            ...
            with psycopg2.connect(self.db_url) as conn:
                with conn.cursor() as cur:
                    insert_query = sql.SQL("""
                        ...
                        RETURNING id
                    """)
                    cur.execute(insert_query, (title, content, 5))  # Assuming 4 is the topic_id for Health
...
def main():
    ...
    keywords = [
        'health research', 'medical breakthrough', 'healthcare innovation', 'public health',
        'disease prevention', 'mental health', 'nutrition', 'fitness', 'wellness',
        'medical technology', 'healthcare policy', 'global health', 'chronic diseases',
        'infectious diseases', 'vaccines', 'cancer research', 'heart health',
        'neurology', 'pediatrics', 'geriatrics', 'women\'s health', 'men\'s health'
    ]

    logger.info(f"Fetching top health-related news articles for {yesterday}...")
    ...
    logger.info("Health news fetching, summarization, HTML generation, and database storage complete.")
