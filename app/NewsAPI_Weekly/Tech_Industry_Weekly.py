...
class TechIndustryNewsFetcher:
    ...
    def fetch_news(self, start_date, end_date, language='en', sort_by='relevancy'):
        ...
        query = ("tech industry OR technology sector OR tech innovation OR "
                 "artificial intelligence OR machine learning OR cloud computing OR "
                 "cybersecurity OR blockchain OR augmented reality OR virtual reality OR "
                 "robotics OR green tech OR tech startups")
    ...
    def filter_and_sort_articles(self, articles):
        def relevance_score(article):
            ...
            priority_topics = [
                'Artificial Intelligence', 'Machine Learning', '5G', 'IoT',
                'Blockchain', 'Quantum Computing', 'Cloud Computing', 'Edge Computing',
                'Cybersecurity', 'Data Privacy', 'Augmented Reality', 'Virtual Reality',
                'Robotics', 'Autonomous Vehicles', 'Electric Vehicles', 'Renewable Energy',
                'Biotechnology', 'Nanotechnology', 'Space Technology', '3D Printing',
                'Semiconductors', 'Chip Shortage', 'Tech Regulation', 'Digital Transformation'
            ]
    ...
    def generate_highlights(self, articles):
        ...
        for article in articles:
            ...
            prompt = f"""
            Based on the following tech industry article title:

            Title: {title}

            1. Create a concise highlight of maximum 130 characters.
            ...
            try:
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system", "content": "You are an AI assistant that creates concise tech industry news highlights and suggests relevant icons and colors."},
                ...
                highlights.append((short_title, "microchip", "#00aaff"))
    ...
    def generate_dynamic_overview(self, highlights):
        ...
        prompt = f"""
        Based on the following tech industry news highlights, generate a catchy and informative overview
        that summarizes the main themes or most significant developments. The overview should
        be engaging and specific to tech industry developments mentioned in the highlights.

        Highlights:
        {', '.join(highlight_texts)}

        Generate an overview in the format: "Tech Industry [Theme]: [Specific Detail]"
        For example: "Tech Industry Breakthrough: Quantum Computing Reaches New Milestone"

        The overview should be 2-3 sentences long, capturing the essence of the week's tech industry developments based on the highlights.
        """
        ...
        try:
            response = client.chat.completions.create(
                model="gpt-4-1106-preview",
                messages=[
                    {"role": "system", "content": "You are an AI assistant that creates engaging overviews for tech industry news summaries."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=200,
                n=1,
            ...
            return "This week in Tech: Groundbreaking innovations and industry shifts reshape the technological landscape."
    ...
    def generate_html_content(self, dynamic_overview, highlights, articles):
        ...
        template = template.replace('{{topic}}', "Tech & Industry")
...
def main():
    fetcher = TechIndustryNewsFetcher()
    today = datetime.now().date()
    last_week = today - timedelta(days=7)
    
    logger.info(f"Fetching top 10 Tech Industry news articles for the week {last_week} to {today}...")
    ...
    if articles:
        ...
        if html_content:
            filename = f"weekly_Tech_Industry_newsletter_{today.strftime('%Y%m%d')}.html"
            ...
            inserted_id = fetcher.store_weekly_newsletter(
                ...
                topic_id=3  # Topic ID for Tech & Industry weekly newsletters
            )
    ...
    logger.info("Weekly Tech Industry news fetching, summarization, and newsletter generation complete.")
