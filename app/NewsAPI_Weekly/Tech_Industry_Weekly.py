...
class TechIndustryWeeklyFetcher:
    def __init__(self):
        ...
        self.trusted_sources = [
            ...
            'engadget.com', 'gizmodo.com', 'techradar.com', 'protocol.com',
            'theinformation.com', 'recode.net', 'theregister.com'
        ]

        self.tech_companies = [
            'Apple', 'Microsoft', 'Google', 'Amazon', 'Meta', 'NVIDIA',
            'Tesla', 'Intel', 'AMD', 'Qualcomm', 'Samsung', 'IBM',
            'Oracle', 'Salesforce', 'Adobe', 'Cisco', 'TSMC',
            'OpenAI', 'Anthropic', 'DeepMind'
        ]

        self.priority_topics = [
            'Artificial Intelligence', 'Cloud Computing', 'Semiconductors',
            'Cybersecurity', '5G', 'Quantum Computing', 'Edge Computing',
            'Blockchain', 'Internet of Things', 'Digital Transformation',
            'Tech Regulation', 'Tech Innovation', 'Green Technology'
        ]
    ...
    async def fetch_news(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        queries = [
            'technology industry OR tech sector',
            'AI development OR cloud computing',
            'semiconductor OR chip manufacturing',
            'tech innovation OR digital transformation',
            'cybersecurity OR data privacy',
            'quantum computing OR edge computing',
    ...
    async def process_articles_with_ai(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            messages = [
                {"role": "system", "content": """Technology news curator. Output JSON:
{
  "filtered_indices": [0-based indices],
  "highlights": [{"text": "max 170 chars", "category": "cat", "icon": "microchip", "color": "#hex"}],
  "title": "Tech Weekly: [Theme]",
  "summary": "2-line technology industry summary"
}"""},
                {"role": "user", "content": f"Select top tech industry news:\n{articles_text}"}
            ]
    ...
    def _process_ai_response(self, response: Any, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        try:
            ...
            return {
                ...
                'title': str(result.get('title', "This Week in Technology")),
                'summary': str(result.get('summary', "Latest technology developments."))
            }
        except Exception as e:
    ...
    def _fallback_processing(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'highlights': highlights,
            'title': "This Week in Technology",
            'summary': "Latest developments in technology, innovation, and digital transformation."
        }
            ...
            with psycopg2.connect(self.db_url) as conn:
                with conn.cursor() as cur:
                    ...
                    """, (
                        title,
                        content,
                        3,  # Tech Industry Weekly topic ID
...
async def main():
    try:
        fetcher = TechIndustryWeeklyFetcher()
        end_date = datetime.now()
        start_date = end_date - timedelta(days=7)
        
        logger.info(f"Fetching tech industry news for {start_date.date()} to {end_date.date()}")
        ...
        if html_content:
            #filename = f"Tech_Weekly_{end_date.strftime('%Y%m%d')}.html"
