...
class BusinessFinanceWeeklyFetcher:
    def __init__(self):
        ...
        self.trusted_sources = [
            'bloomberg.com', 'reuters.com', 'ft.com', 'cnbc.com', 'wsj.com',
            'forbes.com', 'businessinsider.com', 'economist.com', 'barrons.com',
            'marketwatch.com', 'finance.yahoo.com', 'investing.com', 'fool.com'
        ]

        self.business_companies = [
            'Apple', 'Microsoft', 'Amazon', 'Alphabet', 'Meta',
            'Tesla', 'JPMorgan', 'Goldman Sachs', 'BlackRock', 'Morgan Stanley',
            'Bank of America', 'Wells Fargo', 'Berkshire Hathaway'
        ]

        self.priority_topics = [
            'Market Analysis', 'Financial Reports', 'Economic Policy',
            'Stock Market', 'Cryptocurrency', 'Banking', 'Investment',
            'Corporate Finance', 'Trade Policy', 'Business Strategy'
        ]
    ...
    async def fetch_news(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        queries = [
            'business news OR finance news',
            'stock market OR investment trends',
            'economic policy OR financial markets',
            'corporate earnings OR business strategy',
            'banking sector OR financial technology'
        ]
    ...
    async def process_articles_with_ai(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            messages = [
                {"role": "system", "content": """Business news curator. Output JSON:
{
  "filtered_indices": [0-based indices],
  "highlights": [{"text": "max 170 chars", "category": "cat", "icon": "chart-line", "color": "#hex"}],
  "title": "Business Weekly: [Theme]",
  "summary": "2-line financial & business summary"
}"""},
                {"role": "user", "content": f"Select top business news:\n{articles_text}"}
            ]
    ...
    def _process_ai_response(self, response: Any, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        try:
            ...
            return {
                ...
                'title': str(result.get('title', "This Week in Business")),
                'summary': str(result.get('summary', "Latest business developments."))
            }
        except Exception as e:
    ...
    def _fallback_processing(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'highlights': highlights,
            'title': "This Week in Business & Finance",
            'summary': "Latest developments in business, finance, and markets."
        }
            ...
            with psycopg2.connect(self.db_url) as conn:
                with conn.cursor() as cur:
                    ...
                    """, (
                        title,
                        content,
                        2,  # Business & Finance Weekly topic ID
...
async def main():
    try:
        ...
        logger.info(f"Fetching business news for {start_date.date()} to {end_date.date()}")
        ...
        if html_content:
            #filename = f"Business_Weekly_{end_date.strftime('%Y%m%d')}.html"
