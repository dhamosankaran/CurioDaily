...
class StartupInnovationWeeklyFetcher:
    def __init__(self):
        ...
        self.trusted_sources = [
            'techcrunch.com', 'venturebeat.com', 'crunchbase.com', 'inc.com',
            'entrepreneur.com', 'startupgrind.com', 'producthunt.com', 'ycombinator.com',
            'sifted.eu', 'eu-startups.com', 'startupdaily.net', 'startupnation.com',
            'theinformation.com', 'axios.com', 'pitchbook.com', 'fastcompany.com'
        ]

        self.startup_accelerators = [
            'Y Combinator', 'Techstars', '500 Startups', 'Seedcamp',
            'Startup Wise Guys', 'AngelPad', 'Founders Factory', 'Plug and Play',
            'Entrepreneur First', 'Wayra', 'HAX', 'StartX', 'MassChallenge',
            'Alchemist Accelerator', 'SOSV', 'Antler'
        ]

        self.priority_topics = [
            'Startup Funding', 'Seed Round', 'Series A', 'Venture Capital',
            'Innovation Hub', 'Startup Accelerator', 'Tech Innovation',
            'Startup Exit', 'IPO', 'Startup Acquisition', 'Unicorn Status',
            'Entrepreneurship', 'Startup Ecosystem', 'Angel Investment',
            'Disruptive Innovation', 'Growth Strategy', 'Startup Culture'
        ]

        self.innovation_areas = [
            'FinTech', 'HealthTech', 'CleanTech', 'EdTech', 'BioTech',
            'AI Startups', 'Web3', 'Blockchain', 'SpaceTech', 'AgTech',
            'DeepTech', 'Robotics', 'IoT', 'SaaS', 'Marketplace',
            'Consumer Tech', 'Enterprise Tech', 'Digital Health'
        ]
    ...
    async def fetch_news(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        queries = [
            ...
            'startup innovation OR tech startup',
            'startup acquisition OR startup exit',
            'startup accelerator OR incubator',
            'seed funding OR series A',
            'startup ecosystem OR entrepreneurship',
            'disruptive innovation OR innovative startup',
            'unicorn startup OR startup valuation'
        ]
    ...
    async def process_articles_with_ai(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            messages = [
                {"role": "system", "content": """Startup and innovation news curator. Output JSON:
{
  "filtered_indices": [0-based indices],
  "highlights": [{"text": "max 170 chars", "category": "cat", "icon": "rocket", "color": "#hex"}],
  "title": "Startup Weekly: [Theme]",
  "summary": "2-line startup & innovation summary"
}"""},
                {"role": "user", "content": f"Select top startup and innovation news:\n{articles_text}"}
            ]
    ...
    def _process_ai_response(self, response: Any, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        try:
            ...
            return {
                ...
                'title': str(result.get('title', "This Week in Startups")),
                'summary': str(result.get('summary', "Latest startup and innovation developments."))
            }
        except Exception as e:
    ...
    def _fallback_processing(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'highlights': highlights,
            'title': "This Week in Startups & Innovation",
            'summary': "Latest developments in startups, venture capital, and innovation."
        }
    ...
    def generate_html_content(self, title: str, summary: str, highlights: List[Dict[str, Any]], 
        ...
        try:
            ...
            return template.replace(
                ...
                '{{topic}}', "Startup & Innovation Weekly"
            ).replace(
            ...
            with psycopg2.connect(self.db_url) as conn:
                with conn.cursor() as cur:
                    ...
                    """, (
                        title,
                        content,
                        4,  # Startup & Innovation Weekly topic ID
...
async def main():
    try:
        fetcher = StartupInnovationWeeklyFetcher()
        end_date = datetime.now()
        start_date = end_date - timedelta(days=7)
        
        logger.info(f"Fetching startup and innovation news for {start_date.date()} to {end_date.date()}")
        ...
        if html_content:
            #filename = f"Startup_Weekly_{end_date.strftime('%Y%m%d')}.html"
